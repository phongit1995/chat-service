...
export const CallSettingsPanel = ({ open, onClose }: CallSettingsPanelProps) => {
  ...
  return (
    <>
      ...
      <aside
        className={`absolute top-0 right-0 bottom-0 z-40 w-80 max-w-full bg-slate-900/95 backdrop-blur-md text-white shadow-2xl ring-1 ring-white/10 transition-transform duration-300 ${
      ...
      >
        ...
        <div className="px-5 py-4 space-y-5 overflow-y-auto" style={{ maxHeight: 'calc(100% - 60px)' }}>
          <DeviceSection
            label="Microphone"
            perm={micPerm}
    ...
    <div>
      ...
      {empty ? (
        ...
        <select
          ...
          className="w-full bg-slate-800 hover:bg-slate-700 border border-white/10 rounded-lg px-3 py-2 text-sm text-white outline-none focus:ring-2 focus:ring-indigo-500"
        >
          {devices.map((d, i) => (
